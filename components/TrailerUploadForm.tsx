...
export default function TrailerUploadForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const dbRes = await fetch('/api/trailers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: title.trim(),
    ...
    if (!dbRes.ok) {
      ...
      setStatus('error')
      setErrorMsg(error || 'Failed to save trailer')
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      >
        {status === 'uploading' ? 'Uploading…' : 'Upload Trailer'}
      </button>
    </form>
  )
}
