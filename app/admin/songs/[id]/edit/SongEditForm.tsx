...
export default function SongEditForm({ song }: { song: Song }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let thumbnailUrl = song.thumbnail_url
    ...
    if (audioFile) {
      try {
        audioUrl = await uploadToR2(audioFile, setProgress)
      } catch (err: unknown) {
        setStatus('error')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block text-sm text-zinc-300 mb-1">Replace cover art</label>
        ...
        {song.thumbnail_url && !thumbFile && (
          <p className="text-xs text-zinc-500 mt-1">Current cover kept if no new file selected.</p>
        )}
      </div>

      <div>
        <label className="block text-sm text-zinc-300 mb-1">Replace audio file</label>
        <input type="file" accept="audio/*" onChange={e => setAudioFile(e.target.files?.[0] ?? null)}
          className="w-full text-zinc-300 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-zinc-700 file:text-white file:cursor-pointer hover:file:bg-zinc-600" />
        {!audioFile && <p className="text-xs text-zinc-500 mt-1">Current audio kept if no new file selected.</p>}
      </div>

      {status === 'saving' && audioFile && (
        <div>
          <div className="flex justify-between text-xs text-zinc-400 mb-1">
            <span>Uploading audio…</span><span>{progress}%</span>
          </div>
